...
__author__ = "Ray Lai (rebased on Duy' code)"
...
class NICBufferFairness(NICTestBase):
    ...
    def execute(self):
        ...
        traffic = self._start_sender1_recv1_traffic(misc_client_args)
        self.wait_for_traffic(traffic)
        self.baseline_timestamp = int(time.time())
        self.baseline_bandwidth = self._get_baseline_bw()
        time.sleep(2)

        iperf2_p_streams = self.test_control.get("parallel_stream_run", {})
        misc_client_args = self.test_control.get("misc_client_args", {}),
        for p_stream in iperf2_p_streams:
            '''set the user specified num of parallel streams for IPERF2'''
            misc_client_args['parallel'] = p_stream
            traffic = self.start_graph_traffic(self.graph, misc_client_args)
            self.wait_for_traffic(traffic)
            self.validate_bandwidth()

    def _start_sender1_recv1_traffic(self, misc_client_args):
    ...
    def start_graph_traffic(self, graph: IperfGraph, misc_client_args) -> List[Traffic]:
